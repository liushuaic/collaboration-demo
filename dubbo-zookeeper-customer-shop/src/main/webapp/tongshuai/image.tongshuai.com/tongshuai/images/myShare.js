var share = function (a, b) {
    //判断是否是本地
    if (window.location.href.indexOf("file://") >= 0 || window.location.href.indexOf("localhost") >= 0) {
    ...
    // 按钮点击事件统一配置
    $(".shareBtn_qzoneAct").off().on("click", function () {
        shareTo('qzone')
    });
    $(".shareBtn_qqAct").off().on("click", function () {
        shareTo('qq')
    });
    $(".shareBtn_sinaAct").off().on("click", function () {
        shareTo('sina')
    });
    $(".shareBtn_wechatAct").off().on("click", function () {
        shareTo('wechat')
    });
    $(".shareBtn_doubanAct").off().on("click", function () {
        shareTo('douban')
    });
    ...
    function shareTo(stype) {
        ...
        var defaultConfig = {
            shareTitle: document.title, //分享出去的标题
            sharePic: sharePic, //分享出去的图片
            shareUrl: window.location.href, //分享出去的url地址
            shareMsg: "", //页面描述
            shareTalk: ""
        };
        ...
        if (shareParams.shareUrl.indexOf("?") >= 0) {
            shareParams.shareUrl += "&share=1&shareCookie=" + globalCookie + "&longTimeFlag=" + longTimeFlag;
        } else {
            shareParams.shareUrl += "?share=1&shareCookie=" + globalCookie + "&longTimeFlag=" + longTimeFlag;
        }
        ...
        $.ajax({
            url: ajaxUrl + "/hshop-user/gather/gatherShare",
            type: "POST",
            data: shareAjaxParams
        })
        // 跳转前编码一下，防止解析出错
        //qq空间接口的传参
        if (stype == 'qzone') {
            shareParams.shareUrl = encodeURIComponent(shareParams.shareUrl);
            window.open('https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=' + shareParams.shareUrl + '&title=' + shareParams.shareTitle + '&pics=' + shareParams.sharePic + '&summary=' + shareParams.shareMsg + '&summary=');
        }
        //新浪微博接口的传参
        if (stype == 'sina') {
            shareParams.shareUrl = encodeURIComponent(shareParams.shareUrl);
            window.open('http://service.weibo.com/share/share.php?title=' + shareParams.shareTitle + '&sharesource=weibo&url=' + shareParams.shareUrl + '&pic=' + shareParams.sharePic);
        }
        //qq好友接口的传参
        if (stype == 'qq') {
            shareParams.shareUrl = encodeURIComponent(shareParams.shareUrl);
            window.open('http://connect.qq.com/widget/shareqq/index.html?url=' + shareParams.shareUrl + '&sharesource=qq&title=' + shareParams.shareTitle + '&pics=' + shareParams.sharePic + '&summary=' + shareParams.shareMsg + '&desc=' + "");
        }
        //豆瓣的传参
        if (stype == 'douban') {
            shareParams.shareUrl = encodeURIComponent(shareParams.shareUrl);
            window.open('https://www.douban.com/share/service?href=' + shareParams.shareUrl + '&sharesource=douban&name=' + shareParams.shareTitle + '&image=' + shareParams.sharePic + '&text=' + shareParams.shareMsg + '&desc=' + "");
        }
        //生成二维码给微信扫描分享
        if (stype == 'wechat') {
            var hasqrcode = $(".share_box").length
            if (!hasqrcode) {
                $("body").append('<div class="share_box"><div class="share_title">分享到微信朋友圈<a class="share_close">×</a></div><div class="share_qrcode_box"></div><div class="share_foot">打开微信，点击底部的“发现”，使用 “扫一扫” 将网页分享到我的朋友圈</div></div>');
    ...
    if (getRequest().share == 1) {
        ha_glob_coo_cb = function () {
            ...
            if (window.location.href.indexOf("&amp;") >= 0) {
                var shareCookieId = window.location.href.split("shareCookie=")[1].split("&amp;")[0];
                var time = window.location.href.split("longTimeFlag=")[1];
            } else {
                var shareCookieId = getRequest().shareCookie;
                var time = getRequest().longTimeFlag;
            }
            ...
            if (setCookie == (globalCookie + "," + time)) {} else {
                ...
                $.ajax({
                    url: ajaxUrl + "/hshop-user/gather/gatherShareBack",
                    type: "POST",
                    data: backAjaxParams
                })
