...
<script type="text/javascript">
    $(function(){
        $('#m-tabs').bootstrapTable({
            toolbar:'#m-where',
            url:'<%=request.getContextPath()%>/memberrank/queryRememberList.jhtml',
            pagination: true,
            pageList:[5, 10, 20, 50],
            pageSize:5,
            ...
            columns:[
                {checkbox:true},
                {field:'username',title:'用户名',width:225},
                {field:'mrname',title:'会员等级',width:225},
                {field:'email',title:'E-mail',width:225},
                {field:'createdate',title:'创建日期',width:225},
                {field:'isenabled',title:'状态',width:225,formatter:function(value,row,index){
                    if(value == 1){
    ...
    function add(){
        BootstrapDialog.show({
            ...
            title: "新增",
            message: $("<div></div>").load("<%=request.getContextPath()%>/memberrank/toAddRemember.jhtml"),
            ...
            buttons: [{// 设置关闭按钮
                ...
                action: function (dialogItself) {
                    ...
                    $("#myMember").form('submit',{
                        url: "<%=request.getContextPath()%>/memberrank/addRemember.jhtml",
                        type: "post",
                        dataType: "text",
                        async: false,
                        cache: false,
    ...
    function update() {
        var arr = $("#m-tabs").bootstrapTable('getSelections');
        if (arr.length != 1) {
            ...
            BootstrapDialog.show({
                type: BootstrapDialog.TYPE_SUCCESS,
                title: "修改",
                message: $("<div></div>").load("<%=request.getContextPath()%>/memberrank/toAddRemember.jhtml?id=" + arr[0].memberid),
                //size : BootstrapDialog.SIZE_SMALL,//size为小，默认的对话框比较宽
                buttons: [{// 设置关闭按钮
                    label: '关闭',
                    action: function (dialogItself) {
    ...
    function remove(id) {
        ...
        Ewin.confirm({message:"确认要删除选择的数据吗？"}).on(function (r){
            if (r == true){
                $.ajax({
                    url:"../memberrank/delRemember.jhtml",
                    type:"post",
                    data:{"ids":ids},
                    dataType:"json",
                    success:function(data){
